...
/*
      "max-size-buffers", 0, "max-size-time", 5000000000,
      "min-threshold-bytes", 20480, "leaky", 2, NULL);
 */
...
dvb_base_bin_remove_pmt_streams (DvbBaseBin * dvbbasebin, GstStructure * pmt)
{
  ...
  /* parse program-ecm in program descriptors */
  ...
  for (i = 0; i < gst_value_list_get_size (streams); ++i) {
    ...
    --stream->usecount;
    if (!gst_structure_has_field (stream_info, "descriptors"))
      continue;
