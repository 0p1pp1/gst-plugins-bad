  ...
  if (stream) {
    ...
    if (ca_desc
        && gst_mpegts_descriptor_parse_ca (ca_desc,
            &stream_cas_id, (guint16 *) & bstream->ecm_pid, NULL, NULL))
    ...
    GstMpegtsSection * section, const GstMpegtsPMT * pmt)
{
  ...
  if (ca_desc
      && gst_mpegts_descriptor_parse_ca (ca_desc,
          &program->ca_sys_id, &program->ecm_pid, NULL, NULL))
    mpegts_base_add_ecm (base, program->ca_sys_id, program->ecm_pid);
  else {
    program->ca_sys_id = 0;
    program->ecm_pid = 0x1FFF;
  ...
  for (i = 0; i < nbstreams; i++) {
    ...
    if (!_pmt_stream_in_program (program, stream))
      ...
      /* check if ecm_pid has changed */
      const GstMpegtsDescriptor *desc;
      guint16 ca_sys, ca_pid;

      ca_sys = program->ca_sys_id;
      ca_pid = program->ecm_pid;
      desc = gst_mpegts_find_descriptor (stream->descriptors, GST_MTS_DESC_CA);
      if (desc)
        gst_mpegts_descriptor_parse_ca (desc, &ca_sys, &ca_pid, NULL, NULL);

      mpegts_base_remove_ecm (base, program->streams[stream->pid]->ecm_pid);
      mpegts_base_add_ecm (base, ca_sys, ca_pid);
    }
  }
  ...
  for (tmp = program->stream_list; tmp; tmp = tmp->next) {
    MpegTSBaseStream *stream = (MpegTSBaseStream *) tmp->data;
    if (!_stream_in_pmt (pmt, stream) && stream->pid != pmt->pcr_pid)
    ...
    guint16 new_pmt_pid, const GstMpegtsPMT * new_pmt)
{
  ...
  if (!ca_desc ||
      !gst_mpegts_descriptor_parse_ca (ca_desc, &ca_sys, &ca_pid, NULL, NULL))
    ca_pid = 0x1FFF;
  if (oldprogram->ecm_pid != ca_pid) {
    GST_DEBUG ("Different program ecm_pid (new:0x%04x, old:0x%04x)",
        ca_pid, oldprogram->ecm_pid);
    return FALSE;
  }
  ...
  for (i = 0; i < nbstreams; ++i) {
    ...
    ca_desc = gst_mpegts_find_descriptor (stream->descriptors, GST_MTS_DESC_CA);
    if (!ca_desc || !gst_mpegts_descriptor_parse_ca
        (ca_desc, &st_ca_sys, &st_ca_pid, NULL, NULL))
      st_ca_pid = ca_pid;
    if (oldstream->ecm_pid != st_ca_pid) {
      GST_DEBUG
          ("New stream 0x%04x has a different ecm pid (new:%d, old:%d)",
          stream->pid, st_ca_pid, oldstream->ecm_pid);
      return FALSE;
    }
...
mpegts_base_apply_pat (MpegTSBase * base, GstMpegtsSection * section)
{
  ...
  if (old_pat) {
    ...
    for (i = 0; i < old_pat->len; ++i) {
      ...
      MPEGTS_BIT_UNSET (base->known_psi, patp->network_or_program_map_PID);
...
mpegts_base_handle_psi (MpegTSBase * base, GstMpegtsSection * section)
{
  ...
  switch (section->section_type) {
    ...
    case GST_MPEGTS_SECTION_ATSC_MGT:
      ...
      break;
    case GST_MPEGTS_SECTION_ISDB_ECM:
